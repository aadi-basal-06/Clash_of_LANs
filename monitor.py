...
class NetworkMonitor:
        ...
        while self.running:
            try:
                ...
                if packet["type"] == PType.PONG:
                    ...
                    # Filter out monitor itself and other monitors from display
                    self.player_states = {
                        pid: info for pid, info in packet["data"].items()
    ...
    def _display_loop(self):
        while self.running:
            ...
            print(f"| Throughput      : {tput:6.1f} B/s {self._bar(tput, 5000):<20}|")
            print("+------------------------------------------------+")
 
            if self.player_states:
                print("| Visible Players                                 |")
                for player_id, info in list(self.player_states.items()):
    ...
    def _join_server(self):
        join_packet = make_packet(PType.JOIN, {
            "player_id": PLAYER_ID,
            "name": "Monitor",
            "is_monitor": True,          # <-- tells server this is an observer
        })
